 ...
 * whole near sky. 0.57 caps that at r≈210. The completed stars now live in the
 * inter-strand void at r=141–238, so like people and the add-person badge the
 * inner ones do tuck behind the disc at full zoom-out; that is an accepted
 * property of the near void, not a bug. What the floor still guarantees is that
 * you can never zoom out far enough for the disc to eclipse the ENTIRE galaxy.
 *
 ...
 * of completed sections: collapsed pairs live in the VOID_SLOTS, so the only
...
/**
 * VOID SLOTS — where completed (lens, section) pairs go to live.
 *
 * Stars sit off the walk entirely, so finishing a section can never displace
 * the section being worked on. They are exempt from the reveal frontier (see the
 * stars render block): earned progress is not undiscovered territory, so it does
 * not wait for the fog to arrive.
 *
 * These used to sit at r=306–353 out in the four frame CORNERS — past the
 * spiral's outermost strand entirely (SPIRAL_T_END is only r≈297) — which read
 * as glowing dots pinned to the edge of the frame rather than part of the
 * galaxy. They now nestle in the DARK POCKETS BETWEEN the spiral's strands, the
 * same inter-strand void that people and the add-person badge already inhabit,
 * so a completed section feels like it settles into the sky it came from.
 *
 * Each point is the exact MIDLINE between two adjacent strands at its angle:
 * the two interleaved strands are half a turn apart, so successive crossings at
 * a fixed angle are MAX_R/(2·TURNS) = 80px apart and the midpoint is 40px off
 * BOTH strands — never on a strand, by construction. Generated by a golden-angle
 * scatter (so completions spread around the sky, not clustered) with each radius
 * snapped to that midline, then verified. Properties, all against the tightest
 * target 347×735 unless noted:
 *  - radii span 141–238, so every star clears the r=120 avatar disc at the home
 *    framing the user actually lands on. (Like people and the add-person badge,
 *    the inner ones tuck behind the disc only when the sky is zoomed to the
 *    MIN_SCALE floor, where the disc's world radius reaches 120/0.57 ≈ 210 — an
 *    accepted property of everything living in the near void.)
 *  - every star is 40px off the nearest strand (dead center of its pocket).
 *  - nearest approach between two stars is 47px, and to addPersonPoint 83px,
 *    both well past MARKER_CLEAR_RADIUS. The "you" name plate is clear.
 *  - all 11 stay on-screen at 347×735, 375×812, 390×844 and 428×926; the
 *    horizontal frame is the binding constraint, which caps the side pockets
 *    near r≈146 and naturally biases the taller radii toward top and bottom.
 *
 * Like personPoint, these ARE positions in the fog's rotated frame — they were
 * solved against the strand geometry, which is fixed by SPIRAL_PHASE, so the
 * baked coordinates already sit in the real rendered gaps. They pan and scale
 * with the sky. Indexed by PAIR INDEX so a star keeps its spot across visits.
 *
 * 11 slots covers every pair today; a 12th (vedic_deep) wraps via the modulo,
 * which is harmless — a wrapped star just shares a pocket.
 */
const VOID_SLOTS: ReadonlyArray<{ x: number; y: number }> = [
  { x: 0, y: -220.9 }, // N, outer
  { x: 136.4, y: 148.9 }, // SE
  { x: -143.3, y: 86 }, // W
  { x: 130.3, y: -99.9 }, // NE
  { x: -39.3, y: 221.9 }, // S, outer
  { x: -110.8, y: -174.2 }, // NW
  { x: 100.7, y: 182 }, // SE, outer
  { x: -108.1, y: 117.7 }, // SW
  { x: 78.8, y: -215.8 }, // N
  { x: 144.9, y: -188.3 }, // NE, outer
  { x: -1.2, y: 141.1 }, // S, inner
]
...
export function SpiralUniverse({
  ...
  const journey = useMemo(() => {
    ...
    /**
     * COLLAPSED STARS LEAVE THE WALK.
     *
     * They used to be beaded onto the spiral ahead of the active run, which
     * coupled two things that have no business being coupled: every completed
     * section pushed the live section further out, so the reads the user is
     * actually being asked about drifted off screen as they made progress —
     ...
     *
     * Stars now settle into the DARK POCKETS between the spiral's strands, so the
     * active run always opens at one of the two fixed anchors and always lands in
     * the same place on screen. Progress is still legible — the void fills with
     * quiet stars — but it no longer moves the work.
     *
     * Slots are indexed by PAIR INDEX, not by completion sequence, so a star
     * appears in the same pocket on every visit and never migrates as later
     * sections complete. VOID_SLOTS is scattered by golden angle, so
     * consecutive completions land in different sectors of the sky.
     */
    const stars: StarNode[] = []
    for (const p of pairs) {
      if (!done[p.idx]) continue
      const slot = VOID_SLOTS[p.idx % VOID_SLOTS.length]
      stars.push({
        pairKey: p.pairKey,
        key: p.key,
        color: p.color,
  ...
  const glyphs = useMemo<Glyph[]>(() => {
    ...
    for (const armPhase of ARMS) {
      for (let s = 0; s <= NEBULA_SAMPLES; s++) {
        ...
        // + SPIRAL_PHASE keeps the fog locked to the reads and stars beaded on
        // it by spiralPoint — the whole sky turns as one rigid body.
        const theta =
